...
class ReReadingMangaTab extends ConsumerWidget {
  const ReReadingMangaTab({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final controller = ref.watch(reReadingMangaTabProvider);

    return ExtendedVisibilityDetector(
      uniqueKey: const Key('ReReadingMangaTab'),
      child: controller.manga.when(
        data: (data) => data.isEmpty
            ? RefreshIndicator(
                onRefresh: () async => ref.refresh(reReadingMangaTabProvider),
                child: Stack(
            ...
            : RefreshIndicator(
                ...
                child: AutoKeepAlive(
                  child: CustomScrollView(
                    key: const PageStorageKey<String>('ReReadingMangaTab'),
        ...
        error: (err, stack) => CustomErrorWidget(
            err.toString(), () => ref.refresh(reReadingMangaTabProvider)),
      ),
    );
  }
}
