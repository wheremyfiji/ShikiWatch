...
class OnHoldMangaTab extends ConsumerWidget {
  const OnHoldMangaTab({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final controller = ref.watch(onHoldMangaTabProvider);

    return ExtendedVisibilityDetector(
      uniqueKey: const Key('OnHoldMangaTab'),
      child: controller.manga.when(
        data: (data) => data.isEmpty
            ? RefreshIndicator(
                onRefresh: () async => ref.refresh(onHoldMangaTabProvider),
                child: Stack(
            ...
            : RefreshIndicator(
                ...
                child: AutoKeepAlive(
                  child: CustomScrollView(
                    key: const PageStorageKey<String>('OnHoldMangaTab'),
        ...
        error: (err, stack) => CustomErrorWidget(
            err.toString(), () => ref.refresh(onHoldMangaTabProvider)),
      ),
    );
  }
}
