...
class DroppedTab extends ConsumerWidget {
  const DroppedTab({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final controller = ref.watch(droppedTabPageProvider);

    final LibraryLayoutMode currentLayout = ref
        .watch(settingsProvider.select((settings) => settings.libraryLayout));

    return ExtendedVisibilityDetector(
      uniqueKey: const Key('DroppedTab'),
      child: controller.animes.when(
        data: (data) {
          if (data.isEmpty) {
            return RefreshIndicator(
              onRefresh: () async => ref.refresh(droppedTabPageProvider),
              child: Stack(
          ...
          return RefreshIndicator(
            ...
            child: AutoKeepAlive(
              child: CustomScrollView(
                key: const PageStorageKey<String>('DroppedTab'),
                slivers: [
                  SliverToBoxAdapter(
                    child: SearchWidget(
        ...
        error: (err, stack) => CustomErrorWidget(
          err.toString(),
          () => ref.refresh(droppedTabPageProvider),
        ),
      ),
    );
  }
