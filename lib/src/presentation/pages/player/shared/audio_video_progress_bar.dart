...
class ProgressBar extends LeafRenderObjectWidget {
  ...
  const ProgressBar({
    ...
    this.timecodeRanges,
    this.timecodeMarkerColor,
  });

  /// List of ranges to highlight on the progress bar.
  /// Each item is a record with a start and end Duration.
  final List<({Duration start, Duration end})>? timecodeRanges;

  final Color? timecodeMarkerColor;
  ...
  RenderObject createRenderObject(BuildContext context) {
    ...
    return _RenderProgressBar(
      ...
      timecodeRanges: timecodeRanges,
      timecodeMarkerColor:
          timecodeMarkerColor ?? tertiaryColor.withOpacity(0.5),
    );
  }
...
class _RenderProgressBar extends RenderBox {
  _RenderProgressBar({
    ...
    required List<({Duration start, Duration end})>? timecodeRanges,
    required Color timecodeMarkerColor,
    ...
    required TextScaler textScaler,
        ...
        _timecodeRanges = timecodeRanges,
        _timecodeMarkerColor = timecodeMarkerColor,
        _buffered = buffered,
        _onSeek = onSeek,
  ...
  List<({Duration start, Duration end})>? get timecodeRanges => _timecodeRanges;
  List<({Duration start, Duration end})>? _timecodeRanges;
  set timecodeRanges(List<({Duration start, Duration end})>? value) {
    if (_timecodeRanges == value) return;
    _timecodeRanges = value;
    markNeedsPaint();
  }

  Color get timecodeMarkerColor => _timecodeMarkerColor;
  Color _timecodeMarkerColor;
  set timecodeMarkerColor(Color value) {
    if (_timecodeMarkerColor == value) return;
    _timecodeMarkerColor = value;
    markNeedsPaint();
  }
  ...
  void _drawTimecodeRanges(Canvas canvas, Size localSize) {
    if (_timecodeRanges == null ||
        _timecodeRanges!.isEmpty ||
    ...
    final barCenterY = localSize.height / 2;

    final top = barCenterY - (_barHeight / 2);
    final bottom = barCenterY + (_barHeight / 2);

    for (final range in _timecodeRanges!) {
      final start = range.start > _total ? _total : range.start;
      final end = range.end > _total ? _total : range.end;

      if (start >= end) continue;

      final startProp = start.inMilliseconds / _total.inMilliseconds;
      final endProp = end.inMilliseconds / _total.inMilliseconds;

      final startX = (startProp * availableWidth) + capRadius;
      final endX = (endProp * availableWidth) + capRadius;
